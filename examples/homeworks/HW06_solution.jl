...
function illuminate(arr, kg)
	...
	I₁ = 1 .+ cos.(kg .* x .* 2π .+ 1 * 2π / 3)
	I₂ = 1 .+ cos.(kg .* x .* 2π .+ 2 * 2π / 3)
	I₃ = 1 .+ cos.(kg .* x .* 2π .+ 3 * 2π / 3)
...
function forward(img, psf, kg, N_phot=1000)
	I_0, I_1, I_2 = illuminate(img, kg)

	
	return [poisson(conv(I_0, psf), N_phot), 
		    poisson(conv(I_1, psf), N_phot),
			poisson(conv(I_2, psf), N_phot)]
end
...
## Procedure
* First shift the spectrum by `fourier_space_shift` along the first dimension (`circshift`)
...
function reconstruct(psf, Cₙ, fourier_space_shift)
	...
	otf = fft(psf)
	w₋₁ = 0.5 .* circshift(otf, (Δ, 0))
	w₀ = otf
	w₁ = 0.5 .* circshift(otf, (-Δ, 0))

	#Cₙ = Cₙ
	C₋₁ = circshift(Cₙ[1], (Δ, 0))
	C₀ = Cₙ[2]
	C₁ = circshift(Cₙ[3], (-Δ, 0))

	
	res = (w₋₁ .* C₋₁ .+ w₀ .* C₀ .+ w₁ .* C₁ .+ 1f-8^2) ./ (w₋₁ .+ w₀ .+ w₁ .+ 1f-8)

	# calculate similarly the eff_otf but adapt it according to the slidess
	eff_otf = (w₋₁.^2 .+ w₀.^2 .+ w₁.^2 .+ 1f-8^2) ./ (w₋₁ .+ w₀ .+ w₁ .+ 1f-8)

	
	return real(ifft(res)), eff_otf
end
...
function wiener_filter(img, otf, ϵ)
	wiener = conj.(otf) ./ (abs2.(otf) .+ ϵ)
	return abs.(real(ifft(fft(img) .* wiener)))
end
