...
function add_hot_pixels!(img, probability=0.1; max_value=one(eltype(img)))
	for (i, v) in enumerate(img)
		img[i] = rand() < probability ? max_value : v
	end
	return img
end
...
function gaussian_noise_remove(arr; kernel_size=(3,3), σ=1)
	kernel = exp.(-rr2(kernel_size) ./ 2 ./ σ.^2)
	kernel ./= sum(kernel)
	#return arr
...
function median_noise_remove!(arr; kernel_size=(5,5))
	if any(iseven.(kernel_size))
		throw(ArgumentError("kernel_size=$kernel_size contains an even size"))
	end
...
# ╔═╡ 10fc80fe-5c78-40bc-8b57-0ae0cf909271


# ╔═╡ d5813e99-1782-44ac-a399-51222d5e84ef


# ╔═╡ 5232db1b-1821-4589-83a1-22b67554ae3d


# ╔═╡ 5a9ee268-b77b-4fa1-8375-4dfcff1deaec


# ╔═╡ 2445e2a0-4745-4db4-8a92-03ff4471e314


# ╔═╡ cb1e1e74-af4e-46a7-87c5-e18f08ff7d0f


# ╔═╡ 3f03ca1f-1326-49c0-a203-4b5265ece21a
