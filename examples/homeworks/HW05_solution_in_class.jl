...
# ╔═╡ cf7a03cc-7b71-44fd-90b1-b5f60e34bb7d
sum(calc_psf((10, 10), 2))
...
"""
function LR(Y, psf, N)
	Sₙ = ones(eltype(Y), size(Y))

	psf_rev = psf[end:-1:begin, end:-1:begin]
	for i = 1:N
		Sₙ .= Sₙ .* (FourierTools.conv(Y ./ (FourierTools.conv(Sₙ, psf)), 
										psf_rev))
	end
	return Sₙ
end

# ╔═╡ 9cc058c1-0a4a-4067-9ba7-cf61e70f3f58
"""
	LR(measured, psf, N)
...
`N` is the number of iterations.
"""
function LR_fast(Y, psf, N)
	# don't start with copy(Y)
	Sₙ = ones(eltype(Y), size(Y))

	psf_rev = psf[end:-1:begin, end:-1:begin]

	# efficient function for conv
	# use real valued FFT
	my_conv(S, otf) = irfft(rfft(S) .* otf, size(psf, 1))
	# calculate otf once!
	otf = rfft(psf)
	# calculate once!
	otf_rev = rfft(psf_rev)
	for i = 1:N
		Sₙ .= Sₙ .* (my_conv(Y ./ (my_conv(Sₙ, otf)), otf_rev))
	end
	return Sₙ
end
...
psf = ifftshift(calc_psf(size(img), 30));
...
# ╔═╡ 4bca876e-493d-4f7a-9696-2695bb53ff34
rec3 = LR_fast(measured, psf, 200);

# ╔═╡ 65119afd-f2be-4c04-931b-d13cdccb466d
LR(measured, psf, 200);

# ╔═╡ 6ba1619b-036a-480d-b3bb-88ea6f5b259e
rec = LR_fast(measured, psf, iterations);
...
function TV(img::AbstractArray{T, N}, ϵ=T(1e-6)) where {T, N}
	...
	@tullio arr_ex = sqrt(ϵ + abs2(img[i,j]-img[i+1,j]) + 
...
function LR_TV(Y, psf, λ, N)
	...
	otf_rev = rfft(psf_rev)
	for i = 1:N
		# since we add the TV to the loss function we also can add it 
		# to the gradient of the loss function
		∇TV = gradient(TV, Sₙ)[1]
		Sₙ .= Sₙ .* (my_conv(Y ./ (my_conv(Sₙ, otf)), otf_rev) .- λ .* ∇TV)
	end
	return Sₙ
end

# ╔═╡ dbaab19b-c7fe-49c0-af21-896108c04cc9
md"

 $L$ is the Poisson loss function

Old loss was: $L$

New loss is: $L' = L + TV$

By adding those losses, the optimizer has to minimize now both together.

For the minimzation you need the gradients:
$\nabla L' = \nabla L + \nabla TV$ 
"

# ╔═╡ ae7c45b6-c8a7-4fe0-b922-9ab40fcce372
# that can easily take a minute. Zygote is very sloooooooow on the first call
# the \lambda parameter has to be hand optimizer by the user
rec2 = LR_TV(measured, psf, 0.005, 100)
...
# ╔═╡ 3585742e-d18a-4af3-9e6d-85cfa48c4bd6
md"We are using the L-BFGS optimizer"
...
function gauss(I, σ::T, x_offset::T, y_offset::T) where T
	...
	return I .* exp.(-1/2 .* (abs2.(x .- x_offset) .+ 
						  abs2.(y .- y_offset)   ) ./ 
				 (σ^2))
end
...
function init_params(N, measurement, bin_factor, σ_guess)
	...
	# the initial position of the points is more critical
...
res = optimize(f, g!, params_guess, LBFGS(), Optim.Options(iterations=iterations2))
...
# ╔═╡ dc0883d8-52df-4a35-9f47-0102a46152aa
