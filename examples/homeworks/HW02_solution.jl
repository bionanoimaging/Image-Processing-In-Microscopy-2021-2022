...
function to_equal_intensity_tuple(value::T) where T
	...
	red_map(x) = (1 + sin(x .* T(2π) .+ 2 * T(2π) / 3)) / 3
	green_map(x) = (1 + sin(x .* T(2π))) / 3
	blue_map(x) = (1 + sin(x .* T(2π) .+ T(2π) / 3)) / 3
	
	#return (1.0, 0f0, 0)
	return (red_map(value), green_map(value), blue_map(value))
end
...
# ╔═╡ 5b14c5d5-32f6-4507-a297-4ec13c8adf43
...
function to_negative_positive_tuple(value::T) where T
	...
	red_map(x) = x < 0.5 ? 1 / T(0.5) * x : one(T)
	green_map(x) = x < 0.5 ? 1 / T(0.5) * x : 1 - 1 / T(0.5) * (x-T(0.5))
	blue_map(x) = x < 0.5 ? one(T) : 1 - 1 / T(0.5) * (x - T(0.5))
...
function simulate_pixel(n_photons; read_σ=5, offset=0.5, gain=0.1)
	return ((pois_rand(n_photons) + read_σ * randn()) * gain + offset)
end
...
simulated_img = simulate_pixel.(img);
...
begin
	# variance
	variance_arr = var(images, dims=(3,))
